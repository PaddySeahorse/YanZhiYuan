...
This is particularly important for spiking neural network models.

All programs below are built with Abseil Python which gives you extensive command line interface (CLI) help descriptions.
You can access these by using the ``--help`` flag on any tasks below, for instance ``python -m norse.task.mnist --help``.

## Cartpole

This task is a balancing exercise where a controller learns to counter the gravitational force on an upright cartpole. You will need to install OpenAI Gym to provide the simulation environments.
...
This skill is generated from `/tmp/norse_repo/docs/pages/tasks.rst`
